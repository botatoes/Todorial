...
class TodoListViewController: SwipeTableViewController{
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        guard let colorHex = selectedCategory?.color else {
            fatalError("error getting color")
        }
    ...
    func updateNavBar (withHexCode colorHex: String) {
        guard let navController = navigationController?.navigationBar else {
            fatalError("Navigation controller does not exist")
        }
        guard let navBarColor = UIColor(hexString: colorHex) else {
            fatalError("navbar color error")
        }
        navController.barTintColor = UIColor(hexString: colorHex)
        navController.tintColor = ContrastColorOf(navBarColor, returnFlat: true)
        navController.largeTitleTextAttributes = [NSAttributedString.Key.foregroundColor : ContrastColorOf(navBarColor, returnFlat: true)]
